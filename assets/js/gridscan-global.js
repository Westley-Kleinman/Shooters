...
// Global version of GridScan for use without modules
// Assumes THREE and POSTPROCESSING are available globally
...
class GridScan {
  constructor(container, options = {}) {
    ...
    // Default options tailored for Shooters II (Amber/Dark theme)
    this.options = {
      sensitivity: 0.55,
      lineThickness: 1.5, // Thicker for better visibility
      linesColor: '#78716c', // Stone-500 (Lighter for more prominence)
      scanColor: '#d97706', // Amber-600
      scanOpacity: 0.8, // More opaque scan line
      gridScale: 0.1,
      lineStyle: 'solid',
      lineJitter: 0.1,
  ...
  init() {
    ...
    if (options.enablePost && typeof POSTPROCESSING !== 'undefined') {
      ...
      this.bloom = new POSTPROCESSING.BloomEffect({
        ...
        luminanceSmoothing: options.bloomSmoothing
      });
      this.bloom.blendMode.opacity.value = Math.max(0, options.bloomIntensity);

      this.chroma = new POSTPROCESSING.ChromaticAberrationEffect({
        offset: new THREE.Vector2(options.chromaticAberration, options.chromaticAberration),
        radialModulation: true,
        modulationOffset: 0.0
      });

      const effectPass = new POSTPROCESSING.EffectPass(this.camera, this.bloom, this.chroma);
  ...
  setupEvents() {
    this.onResize = () => {
      ...
      this.renderer.setSize(container.clientWidth, container.clientHeight);
      this.material.uniforms.iResolution.value.set(container.clientWidth, container.clientHeight, this.renderer.getPixelRatio());
  ...
  startLoop() {
    ...
    const tick = () => {
      ...
      // Static view - no mouse interaction
